...
export default {
  ...
  methods: {
    ...
    startDrawing(event, pageIndex) {
      ...
      this.transformCanvasToPdfFields();
    },
    finishDrawing(event) {
      ...
      this.pdfFields.push({
        ...
        picInfo: [this.pdfw, this.pdfh, this.pdfx, this.pdfy, parseInt(this.currentPage) - 1],
        id: null
      });
    ...
    transformCanvasToPdfFields() {
      this.pdfFields.forEach((field) => {
        const canvas = document.getElementById(`pdf-canvas-${field.page - 1}`);
        if (!canvas) return;
        // Get the PDF page and canvas dimensions
        const rect = canvas.getBoundingClientRect();
        const canvasScaleX = rect.width / canvas.width;
        const canvasScaleY = rect.height / canvas.height;
        // Calculate PDF dimensions for the current page
        const pdfPage = this.pdfDoc.getPage(field.page - 1);
        //const pdfWidth = pdfPage.getWidth();
        const pdfHeight = pdfPage.getHeight();
        // Transform coordinates from Canvas to PDF
        this.pdfx = field.left / canvasScaleX;
        this.pdfy = pdfHeight - (field.top / canvasScaleY) - (field.height / canvasScaleY);
        this.pdfw = field.width / canvasScaleX;
        this.pdfh = field.height / canvasScaleY;
        console.log("Transformed field for PDF:", field);
      });
    },
