    ...
    <div class="flex">
        ...
        <div class="p-4 sm:ml-60 relative">
            ...
            <div class="fixed bottom-4 right-4 flex items-center space-x-4">
                ...
                <button @click="buttonPressed" class="p-2 bg-green-500 text-white rounded hover:bg-green-700">
                    {{ buttonValue }}
                </button>
    ...
    methods: {
        ...
        finishDrawing(event) {
            ...
            this.pdfFields.push({
                ...
                picInfo: [width * 0.75, height * 0.75, this.startX * 0.66, pdfHeight - this.startY , parseInt(this.currentPage) - 1],
                id: null
            });
        ...
        async loadPdf(base64String) {
            try {
                ...
                // Update global PDF coordinates for the first page
                this.storePdfCoordinates(0);
            } catch (error) {
        ...
        async renderPdfPages() {
            try {
                for (let pageNumber of this.pdfPages) {
                    ...
                    pdfHeight = viewport.height * 0.844;
        ...
        // Store PDF coordinates as global variables
        async storePdfCoordinates(pageIndex) {
            if (!this.pdfDoc) {
                console.error("PDF document is not loaded.");
                return;
            }
            try {
                const pdfPage = await this.pdfDoc.getPage(pageIndex + 1); // PDF.js uses 1-based indexing
                const viewport = pdfPage.getViewport({ scale: 1.0 });

                pdfWidth = viewport.width;
                pdfHeight = viewport.height;

                console.log(`Page ${pageIndex + 1} dimensions: width = ${pdfWidth}, height = ${pdfHeight}`);

                // Example usage of pdfX and pdfY
                pdfX = 0; // Set this to the desired value
                pdfY = pdfHeight; // Y-coordinate starts from the bottom left in PDF

            } catch (error) {
                console.error("Error retrieving PDF page:", error);
            }
        }
    }
};
